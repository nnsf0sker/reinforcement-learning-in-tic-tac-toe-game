...
def smart_permutations(obj: Tuple[int]) -> Iterator[Tuple[int]]:  # TODO: Обобщить int до любого неизменяемого типа данных
    if len(obj) == 1:
        yield (obj[0],)
    else:
...
class Referee:
    ...
    def check_field(self, field: Field, last_step_row: Optional[int] = None, last_step_col: Optional[int] = None):
        if (last_step_row is not None) and (last_step_col is not None):
    ...
    def print_field(self, field: Field):
    ...
    def _get_left_diagonal_line(self, field: Field, row: int, col: int) -> Tuple[int]:
        ...
        return tuple(field[first_cell_n + step*(self.cols_n + 1)]
