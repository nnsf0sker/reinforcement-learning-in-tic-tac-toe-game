...
class Game:
    ...
    def one_game(self, start_position: str, first_player: PlayerAdapter, second_player: PlayerAdapter):
        game_result = None
        first_player.side = 1
        second_player.side = 2
        if start_position == 'random':
            field = list(self._referee.get_random_field())
            x_cells = field.count(1)
            o_cells = field.count(2)
            if x_cells == o_cells:
        ...
        while not game_result:
            step = next_player.next_step(tuple(field))
