...
def free_cells(field: Field) -> Tuple[float]:
    return tuple(-1. if cell else 0.5 for cell in field)
    ...
    def __init__(self, config: Dict[str, Any], referee: Referee):
        ...
        try:
            with open(self._db_path, "r") as db_file:
                ...
                self._mode = 'play'
        except:  # TODO: Бесит ошибка: разобраться!
            self._weight_db = {}
            self._mode = 'train'
